...
bool InNetRTMPStream::FeedData(uint8_t *pData, uint32_t dataLength,
	...
	if (isAudio) {
		...
		if (
				(processedLength == 0) //start of the packet
				&&(dataLength >= 2) //enough data
				&&((_lastVideoCodec != (pData[0]&0x0f)) || ((pData[1] == 0)&&(pData[0] == 0x17 || pData[0] == 0x1C))) //different codec or this is a new h264 codec
				) {
		...
		if ((processedLength == 0) && ((pData[0]&0x0f) == 7 || (pData[0]&0x0f) == 12) && (dataLength >= 6)) {
...
bool InNetRTMPStream::InitializeVideoCapabilities(
	...
	switch (pData[0]&0x0f) {
		...
		case 12:
		{
			/*
			 * map:
			 * iiiiiiiiiii ss <sps> i pp <pps> i vv <vps>
			 *
			 * i - ignored
			 * ss - sps length
			 * <sps> - sps data
			...
			if (((pData[0] >> 4) != 1) || (pData[1] != 0)) {
				WARN("stream: %s; this is not a key frame or not a H265 codec setup request. Ignore it: %02" PRIx8"%02" PRIx8,
						(pStream != NULL) ? (STR(pStream->GetName())) : "",
						pData[0], pData[1]);
				return true;
			}
			pCodecInfo = streamCapabilities.AddTrackVideoH265(
					pData, length, 90000, pStream);
			if (pCodecInfo == NULL) {
				FATAL("Unable to parse SPS/PPS/VPS for input RTMP stream");
				return false;
			}
			break;
		}
